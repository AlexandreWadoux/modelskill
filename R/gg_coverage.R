#' Plot prediction-interval coverage calibration (accuracy plot)
#'
...
#' under-cover observations. This diagnostic is also called an *accuracy plot*
#' in geostatistical uncertainty validation (Goovaerts, 2001). The result is an
...
coverage_intervals <- function(obs, lower, upper, level, na.rm,
  ...
  if (normal_mode) {
    ...
    if (is.null(x)) return(data.frame(nominal = levels, empirical = NA_real_))
    z <- abs((x$obs - x$pred) / x$predictive_sd)
    return(data.frame(nominal = sort(unique(levels)), empirical = vapply(sort(unique(levels)),
