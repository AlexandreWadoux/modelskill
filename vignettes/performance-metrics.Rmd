...
## Why use several prediction metrics?

Continuous predictions can come from machine learning, statistical models,
geostatistics, or process-based models. `modelskill` evaluates the agreement
between paired observations and predictions, independently of the model used to
produce them.

Prediction performance has several components. A model may have small average
error but substantial bias, reproduce the observed pattern while systematically
overpredicting, or have good overall accuracy while failing to reproduce the
observed variability.

For this reason, model evaluation should normally use several complementary
metrics rather than a single number.

Validation metrics should be calculated from independent validation data or an
appropriate resampling procedure.
...
## Start by looking at the predictions
...
# A particularly important distinction: `r2()` versus `R2()`
...
|:--|:--|:--|:--|:--|
| **ME / bias** [`bias()`](../reference/bias.html) | \(\mathrm{ME}=\frac{1}{n}\sum_i(o_i-p_i)\) | 0 | Average signed error. Negative = overprediction; positive = underprediction. | Legates & McCabe (1999) |
| **MAE** [`mae()`](../reference/mae.html) | \(\mathrm{MAE}=\frac{1}{n}\sum_i\lvert o_i-p_i\rvert\) | 0 | Average absolute error in response units. | Willmott & Matsuura (2005) |
| **MSE** [`mse()`](../reference/mse.html) | \(\mathrm{MSE}=\frac{1}{n}\sum_i(o_i-p_i)^2\) | 0 | Average squared error; gives more weight to large errors. | Hodson (2022) |
| **RMSE** [`rmse()`](../reference/rmse.html) | \(\mathrm{RMSE}=\sqrt{\frac{1}{n}\sum_i(o_i-p_i)^2}\) | 0 | Overall error magnitude in response units; sensitive to large errors. | Hodson (2022) |
| **NRMSE** [`nrmse()`](../reference/nrmse.html) | \(\mathrm{NRMSE}=\mathrm{RMSE}/s_o\) | 0 | RMSE relative to the observed standard deviation. | Taylor (2001) |
| **cRMSE** [`crmse()`](../reference/crmse.html) | \(\mathrm{cRMSE}=\sqrt{\frac{1}{n}\sum_i[(o_i-p_i)-\mathrm{ME}]^2}\) | 0 | Error remaining after mean bias is removed. | Taylor (2001) |
| **Pearson correlation** [`correlation()`](../reference/correlation.html) | \(r=\frac{\sum_i(o_i-\bar{o})(p_i-\bar{p})}{\sqrt{\sum_i(o_i-\bar{o})^2\sum_i(p_i-\bar{p})^2}}\) | 1 | Strength and direction of linear association; not agreement. | Legates & McCabe (1999) |
| **Squared Pearson correlation** [`r2()`](../reference/r2.html) | \(r^2\) | 1 | Strength of linear association without its sign; not agreement. | Legates & McCabe (1999) |
| **Model efficiency \(R^2\)** [`R2()`](../reference/efficiency_r2.html) | \(1-\frac{\sum_i(o_i-p_i)^2}{\sum_i(o_i-\bar{o})^2}\) | 1 | 0 = observed-mean benchmark; negative = worse than that benchmark. | Nash & Sutcliffe (1970); Janssen & Heuberger (1995) |
| **SD ratio** [`sd_ratio()`](../reference/sd_ratio.html) | \(s_p/s_o\) | 1 | Below 1 = too little variability; above 1 = too much variability. | Taylor (2001) |
| **CCC** [`ccc()`](../reference/ccc.html) | \(\frac{2c_{op}}{v_o+v_p+(\bar{o}-\bar{p})^2}\) | 1 | Agreement in correlation, mean, and variability. | Lin (1989) |
| **Bias correction factor \(C_b\)** [`model_metrics()`](../reference/model_metrics.html) | \(\frac{2\sqrt{v_ov_p}}{v_o+v_p+(\bar{o}-\bar{p})^2}\) | 1 | Agreement in mean and variability, without correlation. | Lin (1989) |
...
## Extended metric reference

| Metric / function | Equation | Ideal | Simple interpretation | Reference |
|:--|:--|:--|:--|:--|
| **MdAE** [`mdae()`](../reference/mdae.html) | \(\mathrm{median}_i\lvert o_i-p_i\rvert\) | 0 | Typical absolute error; relatively robust to large errors. | Hyndman & Koehler (2006) |
| **RPD** [`rpd()`](../reference/rpd.html) | \(s_o/\mathrm{RMSE}\) | Larger | Error relative to observed standard deviation. | Bellon-Maurel et al. (2010) |
| **RPIQ** [`rpiq()`](../reference/rpiq.html) | \(\mathrm{IQR}(o)/\mathrm{RMSE}\) | Larger | Error relative to the observed interquartile range. | Bellon-Maurel et al. (2010) |
| **SEP** [`sep()`](../reference/sep.html) | \(\sqrt{\frac{1}{n-1}\sum_i[(o_i-p_i)-\mathrm{ME}]^2}\) | 0 | Sample SD of prediction errors after removing bias. | Bellon-Maurel et al. (2010) |
| **RER** [`rer()`](../reference/rer.html) | \(\frac{\max(o)-\min(o)}{\mathrm{RMSE}}\) | Larger | Error relative to observed range; sensitive to extreme values. | Bellon-Maurel et al. (2010) |
| **MAPE** [`mape()`](../reference/mape.html) | \(\frac{1}{n}\sum_i\left\lvert\frac{o_i-p_i}{o_i}\right\rvert\) | 0 | Mean absolute relative error. Undefined when an observation is zero. | Hyndman & Koehler (2006) |
| **MPE** [`mpe()`](../reference/mpe.html) | \(\frac{100}{n}\sum_i\frac{o_i-p_i}{o_i}\) | 0 | Signed relative bias in percent. Undefined when an observation is zero. | Hyndman & Koehler (2006) |
| **sMAPE** [`smape()`](../reference/smape.html) | \(\frac{1}{n}\sum_i\frac{2\lvert o_i-p_i\rvert}{\lvert o_i\rvert+\lvert p_i\rvert}\) | 0 | Symmetric relative absolute error. | Hyndman & Koehler (2006) |
| **MSLE** [`msle()`](../reference/msle.html) | \(\frac{1}{n}\sum_i[\log(1+o_i)-\log(1+p_i)]^2\) | 0 | Squared error on the log1p scale. Requires non-negative values. | Hodson (2022) |
| **RMSLE** [`rmsle()`](../reference/rmsle.html) | \(\sqrt{\mathrm{MSLE}}\) | 0 | Root mean squared error on the log1p scale. | Hodson (2022) |
| **RAE** [`rae()`](../reference/rae.html) | \(\frac{\sum_i\lvert o_i-p_i\rvert}{\sum_i\lvert o_i-\bar{o}\rvert}\) | 0 | Absolute error relative to predicting the observed mean; 1 is the benchmark. | Hyndman & Koehler (2006) |
| **RRMSE** [`rrmse()`](../reference/rrmse.html) | \(100\,\mathrm{RMSE}/\lvert\bar{o}\rvert\) | 0 | RMSE as a percentage of the absolute observed mean. | Willmott et al. (1985) |
| **Willmott's \(d\)** [`willmott_d()`](../reference/willmott_d.html) | \(1-\frac{\sum_i(o_i-p_i)^2}{\sum_i(\lvert p_i-\bar{o}\rvert+\lvert o_i-\bar{o}\rvert)^2}\) | 1 | Index of agreement; sensitive to large errors. | Willmott et al. (1985) |
| **KGE** [`kge()`](../reference/kge.html) | \(1-\sqrt{(r-1)^2+(s_p/s_o-1)^2+(\bar{p}/\bar{o}-1)^2}\) | 1 | Combines correlation, variability ratio, and mean ratio. | Gupta et al. (2009) |

For `MAPE` and `sMAPE`, `modelskill` returns a fraction; multiply by 100 if a
percentage is required. `MPE` and `RRMSE` are returned directly as percentages.
