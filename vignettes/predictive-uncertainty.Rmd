...
many models also quantify how uncertain each prediction is. This inFpitformation may
...
| [`pit()`](../reference/pit.html) | Calculates probability integral transform values from predictive CDF values or directly from predictive means and standard deviations under a normal predictive-distribution assumption. | [@gneiting2007; @schmidinger2023] |
...
weighted predictive samples, while `pit()` can use predictive CDF values evaluated at the observations or calculate
them directly from predictive means and standard deviations when a normal
predictive distribution is assumed. `log_score()` requires predictive density
values evaluated at the observations.
