...
evaluation of quantitative maps and predictions by Wadoux, Walvoort, and Brus
(2022). It is derived from the decomposition of root mean square error into
mean error and centred error.
...
and let SDE denote the standard deviation, or centred component, of the
prediction errors. Then
...
For the diagram these quantities are standardized relative to the variability
of the observations:
...
The resulting geometric relationship is
...
`modelskill` preserves the finite-sample scaling used in the original
implementation. See [`diagram_stats()`](../reference/diagram_stats.html) for
the exact computational definitions.

## Reading a solar diagram
...
The origin therefore represents perfect prediction.
...
Thus, points inside the circle have positive MEC/NSE/uppercase `R2()`.

The pale regions represent **lower bounds on Pearson correlation**, not exact
correlation values. A point inside the \(r>0.9\) region, for example, must have
correlation greater than approximately 0.9, but its precise correlation cannot
be read from the region alone.
...
mean bias contributes directly to position and distance from the optimum.

## Colour models instead of writing labels
...
equivalent to NSE and MEC. This should not be confused with lowercase
[`r2()`](../reference/r2.html), which is squared Pearson correlation.
...
The **target diagram** was developed by Jolliff et al. (2009) as a compact
representation of bias and centred root mean square difference.

Like the solar diagram, it is based on
...
The important difference is that the target diagram gives the centred-error
component a sign according to the relative variability of predictions and
observations:
...
* a **negative signed SDE\*** indicates prediction variability smaller than
  observation variability;
* a **positive signed SDE\*** indicates prediction variability equal to or
  greater than observation variability.

This makes the target diagram useful when it is important to distinguish
predictions that are too smooth from predictions that are too variable.

## Reading a target diagram
...
### Note on the `modelskill` coordinate convention
...
The **Taylor diagram** was introduced by Taylor (2001) to summarize several
...
## A limitation of the Taylor diagram
...
# Customising the diagrams with ggplot2
...
## Change the model colour palette
...
## Zoom without changing the statistics
...
* Jolliff, J. K., Kindle, J. C., Shulman, I., Penta, B., Friedrichs,
  M. A. M., Helber, R., and Arnone, R. A. (2009). Summary diagrams for
